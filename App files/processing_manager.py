...
class ProcessingManager:
    def __init__(self, 
        ...
        try:
            ...
            self.patchcore_metadata = config["model_metadata_path"]
    ...
    def load_model(self):
            ...
            try:
                logger.info(f"Initializing PatchCore model using the path: {self.model_path}")
                self.model = OpenVINOInferencer(
                    path=self.model_path,
                    metadata=self.patchcore_metadata,
    ...
    def process_image_core(self, image_path):
        ...
        try:
            ...
            # Run the model prediction (assuming it returns an anomaly map for heatmap)
            ...
            # If anomaly map is present, overlay heatmap
            if hasattr(prediction, "anomaly_map") and prediction.anomaly_map is not None:
                # Get and resize anomaly map to match image size
                ...
                # Apply a color map (JET for heatmap visualization)
                anomaly_map_colored = cv2.applyColorMap(anomaly_map_uint8, cv2.COLORMAP_JET)
                
                # Convert the original image to BGR for blending
                ...
                predicted_image = Image.fromarray(overlayed_image_rgb)
            else:
                # If no anomaly map, just return the image with a label (fallback)
                predicted_image = image
                logger.warning("No anomaly map found, displaying original image.")

            # Set piece status (nok or ok based on the label)
            piece_status = "nok" if pred_label else "ok"
